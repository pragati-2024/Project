...
const questionData = [
  ...
  { id: 11, name: 'HR Round', description: 'Most common HR questions', category: 'HR/GD', image: gdIcon, path: 'hr' },
...
const QuestionBank = () => {
  ...
  const groupedAndFilteredData = useMemo(() => groupByCategory(filteredData), [filteredData]);

  // On phones, keep sidebar closed by default so it doesn't cover the content.
      ...
      <div className={`flex-1 p-4 sm:p-6 md:p-8 transition-all duration-300 ${sidebarOpen ? 'md:ml-64' : 'md:ml-20'}`}>
        <div className="max-w-7xl mx-auto py-6 sm:py-8">
          <div className="text-center mb-8">
            <h1 className="text-3xl sm:text-4xl font-bold mb-2">Interview Question Bank</h1>
            <p className="text-base sm:text-lg text-slate-600 dark:text-purple-200">Select your topic to view detailed questions</p>
          </div>
        ...
        {Object.entries(groupedAndFilteredData).map(([category, items]) => (
          <div key={category} className="category-row">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {items.map((item) => (
                <div key={item.id} className="bg-purple-600 hover:bg-purple-500 transition rounded-2xl shadow-lg p-6 flex flex-col items-center text-center">
